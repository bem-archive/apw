...
// ================================================
    ...
    subscribeOnAllDone: function(cb) {
        cb && this.allDoneSubscribers.push(cb);
    },

    fireAllDone: function() {
        var id = this.id;

        this.allDoneSubscribers.forEach(function(cb) {
            process.nextTick(function() { cb(id) });
        });
    },
