...
module.exports = INHERIT(EventEmitter, {
    ...
    getOtherJob: function(excludeJobs) {
        for (var i = 0; i < this.jobs.length; i++) {
            if (!excludeJobs[this.jobs[i]]) return this.jobs[i];
        }
    },
    ...
    nextJob: function(id) {
        ...
        if (!this.locked && this.jobs.length) {
            if (id) {
                var i = this.jobs.indexOf(id);
                return (i !== -1) && pushJob(this, this.jobs.splice(i, 1)[0]);
            } else {
                return pushJob(this, this.jobs.shift());
            }
        }
    },
