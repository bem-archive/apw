...
Graph.prototype.setNode = function(node, id, children, parents) {
    var G = this;

    if (!id) id = node.getId? node.getId() : nodeIDSequence++;
    ...
    for (var k in G.plans) {
        G.plans[k].link(id, parents);
        G.plans[k].link(children, id);
    }
...
Graph.prototype.getNode = function(id) {
    ...
    return G.hasNode(id) ? { id: id, node: G.nodes[id] } : null;
};
...
Graph.prototype.createPlan = function(/* ids */) {
    ...
    for (var id in ids) {
        // TODO: error
        if (!G.hasNode(id)) {
            console.error("Don't know how to build '%s'", id);
            continue;
        }
