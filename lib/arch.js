...
module.exports = INHERIT(/** @lends Arch.prototype */ {
    ...
    toJson: function(nodes, children) {
        nodes = nodes || this.nodes;
        children = children || this.children;

        var nodesIds = Object.keys(nodes),
            links = Object.keys(children)
                ...
                .map(function(parent) {
                    return children[parent]
                        .map(function(c) {
                            return UTIL.format('{"source": %s, "target":%s}', nodesIds.indexOf(parent), nodesIds.indexOf(c));
                        }).join(',');
                }).join(',');

        return '{"nodes":[\n' +
            nodesIds.map(function(r) {
                return this.getNode(r).toJson(r);
            }, this).join(',') + '], "links":[' + links +']}\n';
    },
    ...
    nodeToJson: function(id) {
        ...
        traverse(id, nodes, links);
        return this.toJson(nodes, links);

        function traverse(id, nodes, links) {
            links[id] = _this.children[id];
            nodes[id] = _this.getNode(id);
