...
module.exports = INHERIT({
     ...
     * It is possible to list IDs.
     *
     * @param {String} id Child node ID to check.
     * @param {String|String[]} parentId Parent IDs to check.
     * @returns {boolean} True if all IDs are parent for this child, otherwise false.
     */
    hasParents: function(id /*, parentId0, parentId1, ... */) {
        return this._hasLink(id, 'parents', arguments);
    },
     ...
     * @param {String|String[]} childId Child IDs to check.
     * @returns {boolean} True if all IDs are children for this parent, otherwise false.
     */
    hasChildren: function(id /*, childId0, childId1, ... */) {
        return this._hasLink(id, 'children', arguments);
    },

    _hasLink: function(id, links, args) {
        var _links = this[links][id];

        for (var i = 1; i < args.length; i++) {
            var lid = args[1];
            if (typeof lid === 'string') {
                if (_links.indexOf(lid) === -1) return false;
            } else if (Array.isArray(lid)) {
                lid.forEach(function(_lid) {
                    if (_links.indexOf(_lid) === -1) return false;
                });
            }
        }
